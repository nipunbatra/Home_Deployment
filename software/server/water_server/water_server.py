...
urls = ('/upload', 'Upload',
	'/query','query',
	'/','home')
filedir="/home/muc/Desktop/water_data/"
render = web.template.render('templates')
db = web.database(dbn='mysql', db='water', user='root', pw='password')
...
class query:	
	def POST(self):
		...
		node_id=query_data["id"]
		query="SELECT state,timestamp FROM water_data WHERE meter_id="+node_id+" and timestamp BETWEEN "+str(start_time)+" AND "+str(end_time)
		...
		for i in range(0,LEN):
			...
			y[i]=retrieved_data[i]["state"]
			X[i]=datetime.datetime.fromtimestamp(x[i],pytz.timezone(TIMEZONE))
			#print retrieved_data["timestamp"]
		with lock:
			figure = plt.gcf() # get current figure
			plt.axes().relim()
			plt.title("Water consumption vs Time for node"+node_id)
			plt.xlabel("Time")
			plt.ylabel("10 litre events")
			plt.axes().autoscale_view(True,True,True)
			figure.autofmt_xdate()
			plt.plot(X,y,'g.')
			filename=randomword(12)+".jpg"
			plt.ylim((-0.1,1.1))
			plt.savefig("/home/muc/Desktop/Deployment/water_server/static/images/"+filename, bbox_inches=0,dpi=100)
			plt.close()
