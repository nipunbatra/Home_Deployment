...
import random
units={}
units["active_power"]="Watts"
units["reactive_power"]="Watts"
units["voltage"]="Volts"
units["current"]="Amps"
units["frequency"]="Hz"
units["energy"]="Wh"
...
urls = ('/upload', 'Upload',
	'/query','query',
	'/','home')
filedir="/home/muc/Desktop/jplug_temp/"
...
class query:	
	def POST(self):
		...
		mac=query_data["mac"]
		query="SELECT "+parameter+",timestamp FROM jplug_data WHERE mac='"+mac+"' and timestamp BETWEEN "+str(start_time)+" AND "+str(end_time)
		...
		with lock:
			...
			plt.axes().relim()
			plt.title(parameter+" vs Time for mac %s" %mac)
			plt.xlabel("Time")
