...
db = web.database(dbn='mysql', db='ct', user='root', pw='password')
...
class query:	
	def POST(self):
		...
		query="SELECT current,timestamp FROM ct_data WHERE id="+node_id+" and timestamp BETWEEN "+str(start_time)+" AND "+str(end_time)
		...
		with lock:
			...
			plt.savefig("/home/muc/Desktop/Deployment/ct_server/static/images/"+filename, bbox_inches=0,dpi=100)
			plt.close()
